...
META["^TWOII"] = dict(
    full="櫃買市場（OTC）代理 · 元大富櫃50 ETF",
    src="Yahoo Finance · 006201.TWO（<b>代理，非指數本身</b>）",
    formula="近三個月變化率",
    what="上櫃股票以中小型股為主，產業分布比加權指數分散，散戶參與比重也較高。"
         "常被當成台股的「風險偏好溫度計」。",
    how="<b>先講限制</b>：Yahoo 沒有收錄櫃買指數本身"
        "（^TWOII 等代號全部查無此標的），這裡用的是在櫃買市場掛牌、"
        "追蹤富櫃50 的 ETF 當代理。它涵蓋的是櫃買市值前 50 大，"
        "不是全體上櫃股票；而且價格已還原配息，"
        "長期累積報酬會高於價格指數 —— 看「近三個月變化」時影響不大，"
        "但不要拿它的絕對點位去對櫃買指數。"
        "<br>判讀邏輯不變：資金寬鬆、投機氣氛濃時，櫃買往往強於加權；"
        "市場轉保守時先被賣的也是它。兩者走勢持續分歧，"
        "通常是權值股與中小型股之間的資金輪動。")
